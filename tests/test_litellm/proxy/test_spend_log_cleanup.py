...
async def test_the_outstanding_rows_probe_carries_a_statement_timeout():
    """
    The probe runs after the batch loop, so the run budget cannot bound it. If it
    were issued bare, a slow probe would hold a connection past the budget the job
    advertises, which is exactly what the bounds exist to prevent. It must carry
    the same per-statement timeout the delete batches do.
    """
    recorded: list[str] = []
    ...
    remaining = await cleaner._count_remaining(
        mock_prisma_client, datetime.now(timezone.utc) - timedelta(days=7), "LiteLLM_SpendLogs", "startTime"
    )
