...
class SpendLogCleanup:
    ...
    The budget is checked between statements, so a run can overrun it by at most
    one batch timeout. Partition maintenance is the one piece that cannot be cut
    short once started, since dropping a partition is DDL holding an ACCESS
    EXCLUSIVE lock, so it is only started while budget remains.
    """
    ...
    async def _execute_delete_batch(
        ...
        """
        timeout_ms: Final = self._timeout_ms()
    ...
    async def _count_remaining(
        ...
        try:
            async with prisma_client.db.tx() as tx:
                await tx.execute_raw(f"SET LOCAL statement_timeout = {self._timeout_ms()}")
    ...
    async def _clean_spend_log_tables(
        ...
        # Partition maintenance is DDL: it takes an ACCESS EXCLUSIVE lock and cannot
        # be interrupted the way a batch can, so it is only STARTED while the run
        # still has budget. A run that is already over its budget skips it and the
